...
bool Connessione::inviaRaw(void* msg, int length){
	printf("invia connessione... \n");
	int ret = send(this->connId, msg, length,0);
	printf("ret %i\n",ret );
	return (ret != length);
}
bool Connessione::inviaTutti(char* msg){
	int len = strlen(msg) + 1;
	return this->inviaTuttiRaw((void)*msg,len);		
}
bool Connessione::inviaTuttiRaw(void* msg, int len){
	bool errore = false;
	for(Connessione* conn: connessioni)
		errore = errore || conn->inviaRaw(msg, len);
	return errore;
}
